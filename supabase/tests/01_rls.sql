...
update public.profiles set handle = 'alice', display_name = 'Alice A',
  height_cm = 170, age = 45, injuries = array['knee'], dietary = array['gf']
  where id = '11111111-1111-1111-1111-111111111111';
update public.profiles set handle = 'bob', display_name = 'Bob B'
  where id = '22222222-2222-2222-2222-222222222222';
update public.profiles set handle = 'carol', display_name = 'Carol C'
  where id = '33333333-3333-3333-3333-333333333333';

-- Monday 2026-08-24 sits in the Sunday-start week beginning 2026-08-23.
insert into public.events (user_id, date, type, minutes) values
...
insert into public.challenges (week_start) values (date '2026-08-23');
...
select assert_denied(
  $q$insert into public.challenges (week_start) values (date '2026-08-30')$q$,
  'challenges: server-defined reference data must not be writable by clients');

-- a forged inbound request must be impossible
select assert_denied(
  $q$insert into public.friendships (requester, addressee)
...
select assert((select count(*) from public.search_profiles('bo')) = 1,
  'search_profiles: must find a profile by handle prefix');
select assert((select count(*) from public.search_profiles('alice')) = 0,
  'search_profiles: must never return the caller');
select assert((select count(*) from public.search_profiles('b')) = 0,
  'search_profiles: a single character must not enumerate the user table');
select assert(
  pg_get_function_result('public.search_profiles(text)'::regprocedure)
...
select assert(
  (select display_name from public.search_profiles('bob')) = 'Bob B',
  'search_profiles: must return the display name');

-- the leaderboard sums friends' minutes without exposing their events
select assert(
  (select minutes from public.friend_leaderboard(date '2026-08-23')
...
select assert(
  (select count(*) from public.friend_leaderboard(date '2026-08-23')
...
select assert((select count(*) from public.profiles) = 0,
  'profiles: an anonymous visitor must see nothing');
select assert((select count(*) from public.events) = 0,
  'events: an anonymous visitor must see nothing');
select assert_denied(
  $q$select * from public.search_profiles('bob')$q$,
